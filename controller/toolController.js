...
export const getSingleTool = asyncHandler(async (req, res, next) => {
   ...
   const data = await Tool.find({ _id: req.params.id })
   ...
   const user = await User.findById(req.id).populate('wishlist')
   const wishlistIds = new Set(
      user.favorites.map((product) => product._id.toString())
   )

   const toolWithWishlist = {
      ...data._doc,
      isUserFavorite: wishlistIds.has(data._id.toString()),
   }

   res.status(200).json({
      status: true,
      code: 200,
      data: toolWithWishlist,
   })
})
